...
if(types.NOTEX_HOST === window.location.host){
...
function htmlNote(note) {
    ...
    let noteUrl = `http://yuanbiji.com/#/note-${note.noteId}.html`;
    return $(`
        <div class='post-items' id='${note.noteId}'>
            ...
            <p class='readmore' noteId='${note.noteId}'>
                <a href='${noteUrl}' target="_blank" class='editNote iconfont icon-xiugai'></a>
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    switch (message.type) {
        case types.TYPE_GET_NOTES:
            show(message.notes);
            break;
        case types.TYPE_PUSH_NOTE_SUCCESS:
            let alertId = randomString(8);
            var alert = $(`
                <div class='yuanbiji_alert_zz' id="${alertId}">
                    <h3>您的选中已保存至猿笔记</h3>
                </div>
            `);
            $('body').append(alert);
